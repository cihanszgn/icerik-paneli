...
import { scanWebsite, withRetry } from '@/lib/scanners'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const sources = sourceId
      ...
      : await withRetry(() => prisma.source.findMany({ where: { type: 'website', isActive: true } }))

    let clipsFound = 0
    for (const source of sources) {
      if (!source) continue
      clipsFound += await scanWebsite(source)
    }
